...
import { View, Text, TextInput, TouchableOpacity, ScrollView, Image, Modal, ActivityIndicator, Switch,Dimensions } from 'react-native';
...
const categoryColors = [
  '#FF9149', '#27548A', '#60B5FF', '#E9A5F1', '#578FCA', '#7E99A3',
  '#FFC857', '#6A0572', '#38A3A5', '#F26419',
  '#F9C74F', '#90BE6D', '#F3722C', '#F8961E', '#F9C74F',
];
...
const Search = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await axios.get('http://172.19.20.117:5000/ServiceCategories/categories');
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      scrollRef.current?.scrollTo({ x: nextIndex * (width - 60), animated: true });
    }, 3000);
  ...
  const handleSearch = async (text) => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.get('http://172.19.20.117:5000/search/search', {
...
>
  <View style={styles.modalOverlay}>
    <View style={styles.modalCard}>
      ...
      {/* Reset Button */}
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 20 , backgroundColor:'red' , borderRadius: 20, width: '20%',
    height: 50,
}}>
      
        <TouchableOpacity onPress={resetFilters}>
          <Text style={{ color: 'white', fontWeight: 'bold' ,  padding: 20  }}>Reset</Text>
        </TouchableOpacity>
      </View>
    </View>
  </View>
